This section documents exactly what needs to be added when upgrading from the default single-region, single-account model.
No refactor. No rewrite. Only additive changes.
...
Copy code
Hcl
# Secondary region provider (example: us-west-2)
provider "aws" {
  alias  = "secondary"
...
This is intentional explicit duplication, not hidden abstraction.
...
No cross-account state sharing. This is non-negotiable in enterprise environments.
C. Why This Is Designed This Way (Architectural Rationale)
...
Final Positioning Statement (You Can Quote This)
